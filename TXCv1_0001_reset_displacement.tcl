...
set target_shear_strain [expr -1.0*0.00070];	 # Target shear strain (only works for the cyclic case) THIS IS FOR STRAIN CONTROL
...
recorder Node -file pressure2.out -time -node 6 -dof 4 vel;
recorder Element -file stress2.out -time  stress; 
recorder Element -file strain2.out -time  strain; 
recorder Node -file disp2.out -time -node 6 -dof 1 2 3 disp;
...
# puts "VertDisp: [expr $vertDisp] before cyclic loading."
...
analysis 	Transient
		...
		recorder Node -file disp.out -time -node 6 -dof 1 2 3 disp;
...
puts "VertDisp: [expr $vertDisp] after cyclic loading."
puts "Done"
