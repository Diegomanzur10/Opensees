...
set Analysis_case "undrained_monotonic"
	# Options:
	# undrained_cyclic
...
set target_shear_strain [expr 0.3];	 # Target shear strain (only works for the cyclic case) THIS IS FOR STRAIN CONTROL
set frequency     	1; 			 # Hz (only works for the cyclic case)
set devDisp 		-0.3;			 # Deviatoric strain (This only works for monotonic case) THIS IS FOR STRAIN CONTROL
...
################# RECORDERS ##################################
		...
		recorder Node -file pressureMU.out -time -node 6 -dof 4 vel;
		recorder Element -file stressMU.out -time  stress; 
		recorder Element -file strainMU.out -time  strain;
...
constraints 	Penalty 5.e18 3.e1
# constraints Plain
...
eval "timeSeries Path 5 -time {0 20001 21501 1e10} -values {0 1 $eDisp $eDisp}"
pattern Plain 2 5 { 
	sp 5  3	$vertDisp
	sp 6  3	$vertDisp
	sp 7  3 $vertDisp
